...
* You may additionally use some `delay` to make sure to run at least this many
  iterations. If the delay approximately covers the (usually) fast loss drop at
  the beginning of a training run, then the standardization will operate only
  on the flat history region, which can make tolerances more transferable.
